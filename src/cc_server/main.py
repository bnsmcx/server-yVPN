...
def create_user(user: schemas.UserCreate, database: Session = Depends(_get_database)):
    """create a new user"""
    database_user = crud.get_user_by_email(database, email=user.email)
    if database_user:
...
def read_users(skip: int = 0, limit: int = 100, database: Session = Depends(_get_database)):
...
def read_user(user_id: int, database: Session = Depends(_get_database)):
    ...
    database_user = crud.get_user(database, user_id=user_id)
...
def create_endpoint(settings: schemas.EndpointCreate,
    ...
    if not crud.valid_user(database, user_token):
